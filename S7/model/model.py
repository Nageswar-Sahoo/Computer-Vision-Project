...
class CIFRModel(BaseModel):
        def __init__(self, num_classes=10, normalizationtype='Layer', dropout=.01):
            ...
            self.depthwise_separable_conv2 = nn.Sequential(
                ...
                nn.Dropout(.05),
                nn.ReLU(),
                nn.Conv2d(in_channels=512, out_channels=32, kernel_size=1, bias=False),
                nn.BatchNorm2d(32),
                nn.Dropout(.05),
                nn.ReLU(),
            )  # output_size = 28 receptive field  : 5
            ...
            self.convblock3 = nn.Sequential(
                nn.ConvTranspose2d(in_channels=32, out_channels=200, kernel_size=(3, 3), padding=0, bias=False,
                                   dilation=4),
                nn.BatchNorm2d(200),
                nn.Dropout(.05),
                nn.ReLU(),
